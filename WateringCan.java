...
public class WateringCan {
    ...
    public WateringCan(Seeds seedInfo, Exp playerExp) {
    ...
    public void Water(ArrayList<Tile> array, int x) {
        if (array.get(x).getPlowState() == 0) { // NOT YET PLOWED
        ...
        } else if (array.get(x).getWaterCount() <= seedInfo.getWater()
                || array.get(x).getWaterCount() < seedInfo.getBonusWater() + playerExp.getWaterBonus()
                        && array.get(x).getWateredToday() == 0) { // TLDR: if water count does not yet exceed the maximum
                                                             // number of
        ...
        } else if (array.get(x).getWaterCount() <= seedInfo.getWater() || array.get(x).getWaterCount() <= seedInfo.getBonusWater()
            ...
            System.out
                ...
                && array.get(x).getWaterCount() >= seedInfo.getBonusWater()) { // MAX AMOUNT REACHED
