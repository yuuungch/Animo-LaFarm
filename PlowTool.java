...
public class PlowTool {
    ...
    public void Plow(ArrayList<Tile> array, int x) {
        ...
        } else if (array.get(x).getPlowState() == 1 && !array.get(x).getWitherState()) {
            output = "Tile is already plowed!";
        } else if (array.get(x).getPlowState() == 0 && array.get(x).getRockState() == 1) {
            output = "Tile cannot be plowed as there are rocks on it.";
        } else {
            output = "Tile cannot be plowed as there is still a withered plant on it.";
        }
